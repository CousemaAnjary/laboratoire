...
class AuthenticatedUserSessionController extends Controller
{
    public function store(AuthenticatedUserSessionRequest $request)
    {
        ...
        if (Auth::attempt($validated)) {
            ...
            ], 200)->cookie('auth_token',  $token, 60 * 24, '/', null, true, true); // Nom du cookie, Valeur du cookie (token), Durée de vie (1 jour), Chemin (tout le site), Domaine (domaine actuel), Secure (HTTPS uniquement), HttpOnly (inaccessible via JavaScript)
